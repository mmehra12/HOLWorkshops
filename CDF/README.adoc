link:#pre-requisites[*Pre-requisites 2*]

link:++#lab-0---introduction-and-setup++[*Lab 0 - Introduction and setup 2*]

____
link:#verify-access-to-the-workshop-environment[1. Configure permissions in Apache Ranger 2]

link:#kafka-permissions[1.1 Kafka Permissions 2]

link:#schema-registry-permissions[1.2 Schema Registry Permissions 2]

link:#_heading=h.2jxsxqh[1.3 SQL Stream Builder Permissions 2]

link:#_heading=h.z337ya[1.3.1 Kafka - Streaming Analytics Datahub 3]

link:#_heading=h.3j2qqm3[1.3.2 YARN - Streaming Analytics Datahub 4]

link:#_heading=h.1y810tw[1.3.3 Kafka - Streams Messaging Datahub 4]

link:#verify-the-data-providers-in-sql-stream-builder[2. Configure Data Providers in SQL Stream Builder 4]
____

link:#lab-1-create-a-flow-using-the-flow-designer[*Lab 1 : Create a Flow using the Flow Designer 6*]

____
link:#overview[1. Overview 6]

link:#building-the-data-flow[2. Building the Data Flow 9]

link:#create-the-canvas-to-design-your-flow[2.1. Create the canvas to design your flow 9]

link:#step-1-access-the-dataflow-dataservice-from-the-management-console[Step 1 : Access the DataFlow dataservice from the Management Console 9]

link:#step-2-got-to-the-flow-design[Step 2 : Got to the Flow Design 9]

link:#step-3-create-a-new-draft-this-will-be-the-main-process-group-of-your-flow[Step 3 : Create a new Draft ( This will be the main process group of your flow ) 9]

link:#step-4-select-the-appropriate-environment-as-part-of-the-workspace-and-give-your-flow-a-name-and-click-on-create[Step 4 : Select the appropriate environment and give your flow a name and click on CREATE 10]

link:#adding-new-parameters[2.2. Adding new parameters 11]

link:#step-1-click-on-the-flow-options-on-the-top-right-corner-of-your-canvas-and-then-select-parameters[Step 1 : Click on the FLOW OPTIONS on the top right corner of your canvas and then select PARAMETERS 11]

link:#step-2-configure-parameters[Step 2 : Configure Parameters 11]

link:#create-the-flow[2.3. Create the Flow 13]

link:#step-1-add-generateflowfile-processor[STEP 1 : Add GenerateFlowFile processor 14]

link:#step-2-configure-generateflowfile-processor[STEP 2 : Configure GenerateFlowFile processor 14]

link:#step-3-add-putcdpobjectstore-processor[STEP 3 : Add PutCDPObjectStore processor 18]

link:#step-4-configure-putcdpobjectstore-processor[STEP 4 : Configure PutCDPObjectStore processor 18]

link:#section-13[STEP 5 : Create connection between processors 19]

link:#naming-the-queues[2.4. Naming the queues 20]

link:#testing-the-data-flow[3. Testing the Data Flow 22]

link:#step-1-start-test-session[STEP 1 : Start test session 22]

link:#step-2-run-the-flow[STEP 2 : Run the flow 24]

link:#move-the-flow-to-the-flow-catalog[4. Move the Flow to the Flow Catalog 28]

link:#deploying-the-flow[5. Deploying the Flow 28]

link:#step-1-search-for-the-flow-in-the-flow-catalog[Step 1 : Search for the flow in the Flow Catalog 28]

link:#step-2-deploy[Step 2 : Deploy 28]

link:#step-3-select-the-cdp-environment-where-this-flow-will-be-deployed.[Step 3 : Select the CDP environment where this flow will be deployed. 28]

link:#step-4-deployment-name[Step 4 : Deployment Name 28]

link:#step-5-set-the-nifi-configuration[Step 5 : Set the NiFi Configuration 29]

link:#step-6-set-the-parameters[Step 6 : Set the Parameters 29]

link:#step-7-set-the-cluster-size[Step 7 : Set the cluster size 29]

link:#step-8-add-key-performance-indicators[Step 8 : Add Key Performance indicators 30]

link:#step-9-click-deploy[Step 9 : Click Deploy 30]
____

link:#lab-2-migrating-existing-data-flows-to-cdf-pc[*Lab 2 : Migrating Existing Data Flows to CDF-PC 30*]

____
link:#overview-1[1. Overview 30]

link:#running-the-workshop[2. Running the Workshop 36]

link:#create-a-kafka-topic[2.1. Create a Kafka Topic 36]

link:#create-a-schema-in-schema-registry[2.2. Create a Schema in Schema Registry 40]
____

link:#lab-3-operationalizing-externally-developed-data-flows-with-cdf-pc[*Lab 3 : Operationalizing Externally Developed Data Flows with CDF-PC 46*]

____
link:#import-the-flow-into-the-cdf-pc-catalog[1. Import the Flow into the CDF-PC Catalog 46]

link:#deploy-the-flow-in-cdf-pc[2. Deploy the Flow in CDF-PC 47]
____

link:#lab-4-sql-stream-builder[*Lab 4 : SQL Stream Builder 50*]

____
link:#overview-2[1. Overview 50]

link:#running-the-workshop-1[2. Running the workshop 51]

link:#step-1-create-ssb-project[Step 1 : Create SSB Project 51]

link:#step-2-switch-to-the-created-project[Step 2 : Switch to the created project 51]

link:#step-3-create-kafka-data-store[Step 3 : Create Kafka Data store 52]

link:#step-4-create-kafka-table[Step 4 : Create Kafka Table 52]

link:#step-5-configure-the-kafka-table[Step 5 : Configure the Kafka Table 52]

link:#step-6-create-a-flink-job[Step 6 : Create a Flink Job 52]
____
...
image:extracted-media/media/image88.png[extracted-media/media/image88,width=399,height=289]
____
...
image:extracted-media/media/image101.png[extracted-media/media/image101,width=520,height=241]
...
____
NOTE: THESE STEPS HAVE ALREADY BEEN DONE FOR YOU, THIS SECTION WILL WALK YOU THROUGH HOW PERMISSIONS/POLICIES ARE MANAGED IN RANGER. PLEASE DO NOT CHANGE ANYTHING
____
...
image:extracted-media/media/image34.png[extracted-media/media/image34,width=526,height=247]

____
Step 2 : Click on Environments on the left tab

image:extracted-media/media/image53.png[extracted-media/media/image53,width=564,height=221]
...
image:extracted-media/media/image67.png[extracted-media/media/image67,width=624,height=186]
